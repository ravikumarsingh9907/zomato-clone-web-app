...
export default function ReviewCard({ data, follow, loggedInUser}) {
    ...
    useEffect(() => {
        (async () => {
            const reviewLikesResponse = await fetch(`http://localhost:3333/restaurants/${data.brand._id}/reviews/${data._id}/like`, {
                method: 'GET',
            });

            const reviewLikesData = await reviewLikesResponse.json();

            const reviewCommentResponse = await fetch(`http://localhost:3333/restaurants/${data.brand._id}/reviews/${data._id}/comment`, {
                method: 'GET',
            });
    ...
    const handleFollowClick = async (e) => {
        ...
        if (!loggedInUser.error) {
            ...
            if (e.target.parentElement.method === 'post') {
                followResponse = await fetch(`http://localhost:3333/users/${e.target.value}/follow`, {
