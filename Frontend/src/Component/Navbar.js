...
export default function Navbar() {
    ...
    const userGet = async function (e) {
        if (e.target.value) {
            const response = await fetch(`http://localhost:3333/restaurants?query=${e.target.value}`, {
                method: 'GET',
                headers: {
...
export async function loader() {
    ...
    const responseProfilePicture = await fetch(`http://localhost:3333/users/${user._id}/avatar`, {
        method: 'GET',
    });
...
export async function action() {
    const logoutResponse = await fetch('http://localhost:3333/users/logout', {
        method: 'POST',
        headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
